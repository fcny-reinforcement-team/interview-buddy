...
let assistantId: string | null = null;  // Store the assistant ID
let threadId: string | null = null;     // Store the thread ID

// Start the interview (Create Assistant and Thread)
router.post('/start-interview', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const greetingMessage = "Hello, my name is Chet, and I will be giving you your coding interview. Are you ready?";
  ...
  // Handle user responses and provide the first question
  router.post('/ask-question', async (req: Request, res: Response) => {
    ...
    try {
      // Add the user's message to the thread (whether they said "yes" or not)
      await openai.beta.threads.messages.create(threadId, {
        role: "user",
        content: userMessage
      });
      ...
      if (userMessage.toLowerCase().includes("yes")) {
        ...
        const firstQuestion = "Great! Here's your first question: What is a binary search tree and how does it work in ${language}?";
